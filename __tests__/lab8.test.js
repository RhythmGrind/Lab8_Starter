describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const items = await page.$$('product-item');
    let allArePopulated = false;
    for(let i=1; i<items.length;i++){
      let shadow = await items[i].getProperty('shadowRoot');
      let button = await shadow.$('button');
      await button.click();
    }
    ...
    innerText = await innerText.jsonValue();
    if(innerText=='20') {allArePopulated = true;}
    expect(allArePopulated).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i=1; i<items.length;i++){
      ...
      await button.click();
      let innerText = await button.getProperty('innerHTML');
      innerText = await count.jsonValue();
      expect(innerText).toBe('Remove from Cart');
    }
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let innerText = await count.getProperty('innerText');
    innerText = await innerText.jsonValue();
    if(innerText== '0') {allArePopulated = true;}
    expect(allArePopulated).toBe(true);
  }, 10000);
